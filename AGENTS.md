...
### Bundled `personas` + steering-coefficient regime

Two bundled discover-mode manifolds ship under `saklas/data/manifolds/`: `personas` (100 persona archetypes in standard assistant-baselined activation space — drawn from Anthropic's Assistant Axis paper (arXiv 2601.10387) + the Persona Selection Model framing) and `personas-role` (same 100-persona corpora, role-augmented: each node's centroid pooled under that persona's chat-template role substitution at fit time, so the manifold lives in persona-baselined activation space). The two are sibling artifacts for comparative methodology — vanilla pulls on persona-content clusters, role pulls on persona-name-association stereotypes (the role variant's PC1 picks up token-OOD-stereotype signal at the extremes, which is itself an interesting finding). Both materialize into `~/.saklas/manifolds/default/` on session start via `materialize_bundled_manifolds()` (process-scope no-op after the first call, to avoid clobbering CLI-set hyperparams on session re-init).

Recommended steering coefficient is `α ≈ 0.20` (usable range `[0.16, 0.25]`) when bundled `personas` is fitted with its default `max_subspace_dim=8` — the bundled manifold ships pre-fitted with that hyperparam for cross-persona-comparable steering control. At `α=0.20` on a 5-persona test (`caveman`, `hacker`, `oracle`, `tyrant`, `philosopher`), every persona produced fluent on-theme persona-coded prose at 100-token continuations. `α≥0.30` universally collapses to looping / garbage; the regime is genuinely narrow but fluent within it. Per-persona sweet spots vary slightly — `philosopher` peaks at `α=0.25` while the others peak at `α=0.20`; suggest users tune per target.
