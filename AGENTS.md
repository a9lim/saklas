...
- **Share-weighting**: per-layer α is multiplied by a normalized per-layer share — the manifold analogue of vector steering's `||baked_L||` weighting. When the per-model whitener covers every fit layer, the fit bakes a **Mahalanobis share** `share_L = sqrt(Σ_k coords_kᵀ M_R coords_k)` with `M_R = B Σ⁻¹ Bᵀ` (the subspace-restricted analogue of the vector path's `||d||_M` bake score; gated all-or-nothing because `||·||_M` and `||·||_2` scales don't reconcile across layers). Without a whitener it falls back to the Euclidean `share_L = ||centered_node_coords_L||_F`, recovered from the RBF interpolant's exact-at-nodes property. The fitted sidecar records which via `share_metric` (the manifold analogue of the vector `bake` label). The cumulative `Σ_L α · share_L = α` makes the user-facing α-regime layer-count-invariant: a manifold covering 30 layers and one covering 12 behave the same at the same user α, just like vector steering.
