...
The three pipelines, all extracting the same bundled ``happy.sad``
direction:
...
OUT_PATH = Path("/tmp/happy_sad_three_way.json")
...
SCENARIO_PROMPT_TEMPLATE = (
    ...
    "assume any emotional valence and should be the kind of question a "
...
def main() -> None:
    ...
    scenarios, statements = load_happy_sad_pack()
    print(
        f"\nbundled happy.sad: {len(scenarios)} scenarios, "
    ...
    session.steer("happy_sad_ctx", ctx_prof)
    ...
    conditions: list[tuple[str, str | None]] = [
        ...
        ("ctx_pos",     f"{ALPHA} happy_sad_ctx"),
        ("ctx_neg",     f"-{ALPHA} happy_sad_ctx"),
