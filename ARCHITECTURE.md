...
There are five `fit_mode`s: `authored` (user gives domain + coords; curved),
`pca` / `spectral` / `auto` (discover — labeled corpora only, coords derived
per-model; `pca` flat, `spectral` curved, `auto` selects flat/curved/periodic),
and `baked` (corpus-less, a precomputed direction from `manifold bake`). The
first four flow through the fit pipeline; `baked` ships an already-fit tensor.
...
- **`LayerSubspace`** — one layer's reduced frame. `mean` (D,), `basis` (R, D);
  ...
  needs). Raw curved fits may also carry `sigma_rbf_weights` /
  `sigma_poly_coeffs`, a separate log-σ RBF for the learned within-node tube;
  `has_sigma` and `sigma_at` are its runtime interface. `is_affine` ⇔
  `node_params is None`. A flat layer additionally carries
...
- **Fuzzy-manifold σ-field** (raw curved fits only) — retained activation rows
  are projected into the fitted subspace, their normal-complement variance is
  reduced to one local thickness per node, and a separate RBF fits `log σ(z)`.
  SAE and affine fits skip this field. It is consumed by soft `onto` and by the
  monitor's assignment/membership reads; the sidecar summarizes it as
...
- **along (`a ∈ ℝ`)** — **translate** the projected foot by the fixed
  ...
  translating.
- **onto (`o ∈ [0,1]`)** — without a σ-field, scale `H_n` by `(1 − o)` and
  collapse the off-surface in-subspace residual onto the zero-thickness wire.
  With a fuzzy σ-field, shrink only the residual magnitude outside the local
  tube toward `σ(z)`: `o=1` lands one σ off the wire, preserves direction, and
  never expands a point already inside the tube. Vacuous when the surface fills
  its subspace.
...
fixed tensor-op sequence is capturable. StaticCache is initialized before a
compiled prefill and reused/reset in place so its K/V identities stay stable.
Persistent offset/capture hooks work in compiled CUDA and MPS sessions;
dynamic/curved/gated hook topologies route to eager, and stream workers stay
eager because CUDA graph trees are thread-local.
...
- **Merged affine subspace** (`add_subspace`): `eff_along_L = share_L ·
  ...
  offset magnitude).
- **Curved manifold** (`add_manifold`): the user-facing `along` and `onto` inputs
  are first clamped to `[0,1]`. For non-periodic domains,
  `eff_along_L = along · share_L · _MANIFOLD_ALONG_GAIN`; for a periodic
  `BoxDomain`, share weighting is deliberately dropped and
  `eff_along_L = clamp(along · _MANIFOLD_ALONG_GAIN, 0, 1)` uniformly across
  layers so no layer wraps past the target at a different rate. In both cases,
  `eff_onto_L = clamp(onto · share_L · _MANIFOLD_ONTO_GAIN, 0, 1)`.
...
There is **no effective `[0,1]` clamp and no water-fill on affine or non-periodic
curved `along`**: a high-signal layer is meant to overshoot, the de-rogued
whitened coords keep the overshoot controlled, and on the curved path
`norm_cap = 3·‖h‖` is the only backstop (the affine fast path relies on the
controlled coords alone). Periodic domains are the intentional exception above.
`onto` keeps its clamp
...
`assignment` + `membership` + depth statistics, plus the `*_per_layer` traces
and optional inspector coordinates) for flat and curved probes alike — the read-side
...
Every full reading also carries the soft view of that geometry. `assignment` is
a top-N node posterior from
`softmax(−d²/(2τ²) − R·log τ)`, including the Gaussian log-volume correction;
curved bandwidths come from the σ-field in the whitened metric and flat
bandwidths from local node spacing. `membership = exp(−residual²/(2σ²))` is the
curved tube-fit density (`1.0` for affine/no-σ fits). `depth_com` and
`depth_spread` report each coordinate axis's share-weighted center and spread in
normalized model depth. `subspace_coords_per_layer` is opt-in inspector data in
the whitened frame, enabled only while trajectory persistence is requested.
...
`"<name>~<label>"` per soft-assignment probability, and
`"<name>:membership"` for tube density. Under the unified full reading every
probe — flat and curved —
...
`@when:<probe>[<i>]` grammar), a subspace fraction (`"emotions:fraction"`), a
label similarity (`"emotions@happy"`), a soft assignment
(`"personas~hacker"`), or tube membership (`"emotions:membership"`). Every
shape composes with a leading `<ns>/`
...
demand via `plan(prep) -> InstrumentPlan` and the session planner unions
...
`InstrumentBinding` (probe specs frozen from the prep; the SAE binding resolves
the normalization unit at bind, immunizing a running generation against the
...
process-wide sequence. Idle-passthrough reads are coherent too: the lens pairs
lens + specs in one locked `_measurement_state()` hold, and the SAE family has
a sibling registry lock behind per-call idle spec snapshots (its metadata
backfill replaces whole spec dicts atomically).
...
- **`base_gain` magnitude.** Steering now uses **three** constants:
  ...
  `_MANIFOLD_ONTO_GAIN = 0.5` (onto / off-surface collapse only). All three are committed but
