...
- **[`manifold_steering.py`](manifold_steering.py)**: author a small cyclic manifold (Russell's affective circumplex), fit it against the model, and generate the same prompt at several positions around the loop. It shows how spline-based manifold steering differs from a single linear direction.
