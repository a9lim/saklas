...
- `vector manifold`: nested `fit` / `discover` / `generate` / `ls` / `show`. `fit <folder> [-m MODEL] [--sae RELEASE] [--sae-revision REV]` loads a model and runs `ManifoldExtractionPipeline` via `session.extract_manifold` on an authored folder. `discover <name> [-m MODEL] [--method pca|spectral] [--max-dim N] [--var-threshold T] [--k-nn K] [--bandwidth SIGMA] [--sae REL]` does the same for a discover-mode folder, writing any CLI override into `manifold.json` (atomically, via `write_json_atomic`) *before* the fit so the cache key reflects the actual inputs; `_sanitize_hyperparams` drops cross-method keys at the IO boundary. `generate <name> --concepts C... [--n-scenarios N] [--statements-per-concept K] [-m MODEL] [-f]` LLM-authors a discover folder by wrapping `session.generate_concept_statements` (one scenario call + N×K cell calls) and writing the corpora + a `scenarios.json` provenance file; output folder is `~/.saklas/manifolds/<ns>/<name>/` ready for `discover` to fit. No `discover --generate` sugar — two-step is deliberate so a flaky generation leaves inspectable corpora. `ls [--namespace NS] [-j]` and `show <name> [-j]` are pure-IO walks of `manifolds_dir()`; `show` surfaces `fit_mode` + `hyperparams` + a per-fit one-line diagnostics summary (PCA: `picked_k=k, cumvar@k=…`; spectral: `picked_k=k, gap=…, bandwidth=…, k_nn=…`) and the derived per-model coords when a discover fit exists.
