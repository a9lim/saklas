...
- `vector manifold`: nested `fit` / `discover` / `generate` / `ls` / `show`. `fit <folder> [-m MODEL] [--sae RELEASE] [--sae-revision REV]` loads a model and runs `ManifoldExtractionPipeline` via `session.extract_manifold` on an authored folder. `discover <name> [-m MODEL] [--method pca|spectral] [--max-dim N] [--var-threshold T] [--k-nn K] [--bandwidth SIGMA] [--max-subspace-dim R] [--sae REL]` does the same for a discover-mode folder, writing any CLI override into `manifold.json` (atomically, via `write_json_atomic`) *before* the fit so the cache key reflects the actual inputs; `_sanitize_hyperparams` drops cross-method keys at the IO boundary. `--max-subspace-dim R` (default 64) caps the per-layer PCA subspace `fit_layer_subspace` uses — smaller R = finer-grained steering at large K (each axis the RBF moves along is one `subspace_replace` displaces), so for steering use set near the manifold's intrinsic `picked_k` (the bundled `personas` ships fitted at `R=8`); keep at 64 for representational analysis. `generate <name> --concepts C... [--n-scenarios N] [--statements-per-concept K] [--role-per-node] [-m MODEL] [-f]` LLM-authors a discover folder by wrapping `session.generate_statements` (one scenario call + N×K cell calls, `share_moment=False` since discover-mode wants per-cell independent statements) and writing the corpora + a `scenarios.json` provenance file; `--role-per-node` doubles each concept slug as that node's assistant-role substitution at fit time, producing a persona manifold (engine validates the slug; family-unsupported raises at the matching `discover` call). Output folder is `~/.saklas/manifolds/<ns>/<name>/` ready for `discover` to fit. No `discover --generate` sugar — two-step is deliberate so a flaky generation leaves inspectable corpora. `ls [--namespace NS] [-j]` and `show <name> [-j]` are pure-IO walks of `manifolds_dir()`; `show` surfaces `fit_mode` + `hyperparams` + a per-fit one-line diagnostics summary (PCA: `picked_k=k, cumvar@k=…`; spectral: `picked_k=k, gap=…, bandwidth=…, k_nn=…`), the derived per-model coords when a discover fit exists, and per-node `[role=<slug>]` tags when set.
