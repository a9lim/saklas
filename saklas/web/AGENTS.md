...
- **`/manifolds`** — `GET` list (with `fit_mode` / `hyperparams` / `fitted_for_session` / `stale` per manifold), `GET /{ns}/{name}` detail (nodes + statements; discover-mode fits carry derived coords + `diagnostics`), `POST` create (authored), `POST /discover` create (discover-mode, label-only nodes), `POST /generate` (SSE; LLM-authors a discover folder via `session.generate_concept_statements`), `PATCH /{ns}/{name}`, `DELETE /{ns}/{name}`, `POST /{ns}/{name}/fit` (SSE; accepts discover-mode `fit_mode` / `hyperparams` overrides on discover folders). Steering a fitted manifold is just a `%` term in the WS `steering` string — no separate route.
...
`ManifoldBuilderDrawer` has top-of-form `authored` / `discover` tabs. **Authored** is the historical two-step (domain → node editor with coords + statements; validated live against `min_nodes = 2n+1` and in-domain coordinates; Save posts to `POST /manifolds`). **Discover** is concept-list + method-picker: a textarea of whitespace/comma-separated concept slugs, `n_scenarios` + `statements_per_concept` sliders, a `pca` / `spectral` toggle with method-specific hyperparams (`var_threshold` for PCA; `k_nn` + `bandwidth` blank-means-auto for spectral), and a `fit immediately after generating corpora` checkbox. Save calls `apiManifoldGenerateStream` (SSE-streamed corpus generation) then optionally chains `apiManifoldFitStream` with the same hyperparams — both legs use sticky progress toasts. The diagnostics renderer in `lib/manifolds/diagnostics.ts` is pure helpers (`classifyDiagnostics`, `pcaBars`, `spectralBars`, `diagnosticsSummary`, `pickDiscoverFit`); the `DiagnosticsPanel.svelte` shell consumes them.

A racked manifold renders as a `ManifoldStrip` in `SteeringRack` (below the vector strips) carrying an `XYPad` position picker, a blend slider, and a trigger pill; the position serializes into the steering expression as a `%` term. The strip carries a purple left-stripe + purple name color to distinguish manifold rows from vector rows at a glance (matches the TUI's `ansi_magenta` manifold-row color).

`ManifoldStrip` carries a "snap to node" `<select>` above the `XYPad`. Picking a node from the dropdown switches the term to **label form** (`<m>%<label>`) by setting `ManifoldRackEntry.label`; the coords mirror the node's authoring coords so the pad still renders the position. Pulling on the pad authors a free-form coord and clears `label`. `setManifoldLabel(name, label|null)` is the store action; `formatManifoldTerm` emits `<m>%<label>` when `label` is set, otherwise the comma-joined coord list. `lib/expression.ts`'s parser produces `ManifoldTerm.label` (string) or `ManifoldTerm.coords` (number[]) and either rides into the rack entry. Each option in the dropdown shows the node's optional `role` tag (`pirate [role=pirate]`) when set, surfacing persona manifolds.

`ManifoldBuilderDrawer`'s authored tab grows a per-node `role` input alongside coords (validated client-side against the same `[a-z0-9._-]+` slug regex the engine uses); the discover tab grows a "persona manifold (use each concept slug as that node's role)" checkbox that sets `GenerateManifoldRequest.role_per_node`. `ManifoldDrawer` rows carry a `persona` `fit-badge` when any node in `node_roles` is non-null, parallel to the `pca` / `spectral` badges.
