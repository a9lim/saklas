...
## Known server drift (4.0)

The 4.0 collapse (vector → manifold) removed surfaces the committed dashboard
bundle still references; these client artifacts are stale until the webui is
re-migrated and rebuilt:

- **Pack routes are gone.** The server no longer serves `GET/POST /saklas/v1/packs`,
  `/packs/search`, or `DELETE /packs/{ns}/{name}`. The client `packsState` /
  `refreshPacks` / `LocalPackInfo.has_tensor` plumbing and `PackDrawer` /
  `ManifoldPacksDrawer`'s pack tab target them and now 404. Concept install/browse
  moved to the manifold routes; vector extract/merge stay under `/sessions/{id}/*`.
- **Cloning is gone.** The server dropped `POST /sessions/{id}/vectors/clone`
  (corpus-cloning was removed in the A2 collapse). The client `apiVectors.clone` /
  `apiCloneStream` (`api.ts`) + `CloneVectorRequest`/`CloneVectorResponse` and
  `CloneDrawer` target it and now 404 — drop them from the webui.
- **`POST /extract/preview` is gone.** A2 conversational extraction has no
  scenario/preview machinery, so the server deleted the preview route. The client
  `apiVectors.previewPairs` (`api.ts`) and the ExtractDrawer `generate previews`
  flow that hits it now 404 — rework ExtractDrawer to the no-preview submit path.
- **One-shot text scoring is gone.** The server deleted `POST .../probe` and
  `POST .../manifold-probe` (they re-rendered arbitrary text via the now-deleted
  `monitor.measure` / `session.measure_manifold`). The client `apiProbes.score`
  (`api.ts`, → `${SESSION}/probe`) + `ScoreProbeRequest`/`ScoreProbeResponse` now
  404 — live per-token scoring (the WS `scores` co-stream, the manifold-probe
  reading extensions, and the traits SSE) is the only scoring path and is
  unchanged. Drop the dead `score` client.
- **The manifold-builder generate wire renamed.** `POST /manifolds/generate` now
  takes `kind: "abstract" | "concrete"` + `samples_per_prompt: int` (per-concept
  conversational responses), not `n_scenarios` / `statements_per_concept`. The
  client `GenerateManifoldRequest` (`types.ts`) + `ManifoldBuilderDrawer`'s
  auto-generated tab still send the old scenario-count fields — re-key them.
- **`:pca` variant is gone.** The Python `_VARIANT_REGEX` rejects it, so any `pca`
  expression `lib/expression.ts` or the `VectorStrip` variant menu still emits is
...
webui/src/
  ...
  panels/
    ...
    InspectorPanel.svelte     # right rack: steering + probe racks splitting the column (probe rack fuses vector + manifold probes; mirrors SteeringRack's vector + manifold layout)
    ...
    ProbeRack.svelte          # fused manifold + vector probe strips (manifolds first), sorted within each family, with "+ add probe" / "+ add manifold probe" footer
...
`ManifoldMergeDrawer` and `ManifoldPacksDrawer` are the peer-of-`ManifoldDrawer` tools, parallel to vectors' `MergeDrawer` and `PackDrawer`. Both live under the workspace rail's `Steering manifolds` category (the rail-spline-icon button next to `Steering & vectors`) so the entry points feel symmetric with the vector side; `ManifoldDrawer` itself stays reachable only from the rack `+` buttons, exactly like `VectorsDrawer`. **`ManifoldMergeDrawer`** is the discover-only node-union surface — checkbox list of discover-mode catalog rows, target-name input, and a `fit_mode` picker that defaults to inherit-from-sources when they agree and requires an explicit override when they don't, with a yellow warning naming the mixed modes. Submits `apiManifolds.merge` and closes back to the catalog on success. **`ManifoldPacksDrawer`** is the local-catalog + HF-search surface — two tabs mirroring `PackDrawer` pixel-for-pixel (installed local rows from the shared `manifoldRack.catalog` store; HF search debounced at 300 ms against `apiManifolds.search`, per-row `install` button calling `apiManifolds.install` and refreshing the local list on success). Both new drawers' purple action accents echo the manifold-family colour the rack and steering strips already use.

`ManifoldBuilderDrawer` has top-of-form `auto-generated` / `custom nodes` tabs (the internal `AuthoringMode` values stay `discover` / `authored` — only the labels rename). **Custom nodes** is the user-authored path: node editor with labels + statements + optional per-node coords, validated live against `min_nodes = 2n+1` and in-domain coordinates; Save posts to `POST /manifolds`. **Auto-generated** is concept-list + method-picker: a textarea of whitespace/comma-separated concept slugs, `n_scenarios` + `statements_per_concept` sliders, a `pca` / `spectral` toggle. (**Stale vs. 4.0:** `POST /manifolds/generate` no longer takes scenario counts — A2 conversational extraction sends `kind: "abstract"|"concrete"` + `samples_per_prompt` per concept; the `n_scenarios` / `statements_per_concept` sliders and `GenerateManifoldRequest` fields need re-keying. See "Known server drift".) Both tabs share an `AdvancedSection` carrying hyperparams (`max_dim`, `var_threshold` for PCA / `k_nn` + `bandwidth` blank-means-auto for spectral) and toggles (`fit immediately after generating corpora`, `persona manifold (use each concept slug as that node's role)`, `overwrite an existing manifold`). Auto-generated save calls `apiManifoldGenerateStream` (SSE-streamed corpus generation) then optionally chains `apiManifoldFitStream` with the same hyperparams — both legs use sticky progress toasts. The diagnostics renderer in `lib/manifolds/diagnostics.ts` is pure helpers (`classifyDiagnostics`, `pcaBars`, `spectralBars`, `diagnosticsSummary`, `pickDiscoverFit`); the `DiagnosticsPanel.svelte` shell consumes them.
...
A racked manifold renders as a `ManifoldStrip` in `SteeringRack` (below the vector strips) carrying an `XYPad` position picker, a blend slider, and a trigger pill; the position serializes into the steering expression as a `%` term. The blend slider authors the manifold `along` coefficient; the engine's `%` slot is `along[,onto]`, but the webui does not yet expose the second `onto` control (a curved-manifold-only collapse fraction), so a pasted `…%pos,onto` expression round-trips but can't be edited from the strip. The strip carries a purple left-stripe + purple name color to distinguish manifold rows from vector rows at a glance (matches the TUI's `ansi_magenta` manifold-row color). The picker is always rendered (no collapse toggle) — manifold steering is unusable without seeing the coords, so hiding them by default was busywork.
...
The read-side surface lives inside the unified `ProbeRack` (one of two equal sections in `InspectorPanel`, peer of `SteeringRack`). Manifold probe strips render at the *top* of the rack, sorted alphabetically — the bundled `personas` / `pad` probes auto-load there (see "Default manifold probes" below), so they head the list. Vector probe strips follow, sorted by the rack's `sortMode` (name / value / change). The value/change ordering doesn't apply across the two probe families (different units, different domains). The rack footer carries two entry-point buttons: `+ add probe` opens `VectorsDrawer`; `+ add manifold probe` opens `ManifoldDrawer` with `params={mode: "probe"}`. When the server doesn't expose the read-side routes (`manifoldProbeRack.unavailable`) the manifold-probe button hides; the rack still renders the vector half. `ManifoldProbeStrip` chrome mirrors the steering `ManifoldStrip` family — purple left-stripe + purple name color — so the manifold surfaces read as one feature across steering and reading. Each strip carries: the EV-weighted `fraction` bar (∈ [0, 1], one matmul per layer in the engine), a sparkline of fraction history (capped at the same 60-tick budget as vector probes), the top-1 nearest-node label + distance for inline readout, and the settled `coords` from the end-of-gen aggregate when present. For ≤2D `BoxDomain` probes (intrinsic_dim == 2, domain.type == "box", `node_coords` populated) the strip also renders `ManifoldMiniMap` — a 168px SVG square with the manifold's node labels placed at their authoring coords, a translucent purple polyline overlay of the inferred per-token trajectory (each token's `nearest[0]` looked up in `node_coords`, capped at 240 points), a small live-cursor dot on the trajectory tail while streaming, and a bold purple-with-white-outline settled dot at `aggregate.coords` once the `done` event lands. Higher-dim and sphere/custom probes attach the same way but skip the mini-map — their reading lives in the text-only `coords (...)` readout.

**Default manifold probes.** `saklas serve` auto-attaches the bundled manifolds (`default/personas`, `default/pad`) as probes on startup when the dashboard is mounted (`_attach_default_manifold_probes` in `cli/runners.py`, gated on `web_enabled` so `--no-web` skips it) — the read-side analogue of the bundled *vector* probes loaded at session construction. It iterates every bundled `default/<name>` manifold, so any fitted concept axis attaches too. Only manifolds already *fitted* for the loaded model attach; an unfitted bundled manifold is skipped with a one-line startup hint (fitting runs a forward pass per node and would block startup, so it stays a manual dashboard action). The client doesn't initiate this — `refreshManifoldProbeList()` simply fetches the server's already-attached set on load, so the rack opens populated.
...
`lib/expression.ts` parses `%` manifold terms (`<manifold>%<coord_list>`, e.g. `0.7 pad%0.3,0.8,0.0@response`) alongside vector terms — `Term` is a discriminated union, `serializeExpression(vectorRack, manifoldRack?)` emits both, `parseExpression` returns `{vectors, manifolds}`. Keep its variant list in parity with the Python grammar (`raw`, `sae`, `sae-*`, `role`, `role-*`, `from-*`) so pasted canonical expressions hydrate instead of being rejected client-side. **The webui still offers a `pca` variant here and in the `VectorStrip` variant menu — stale vs. 4.0: the grammar dropped `:pca` (the Python `_VARIANT_REGEX` rejects it), so a `pca` expression is now server-rejected; drop it from the webui.**
