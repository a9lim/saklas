...
empty always continues and a modifier suppresses generation only for non-empty
text. `type:"generate"` remains for specialist fork/prefill and compatibility.
...
The current cast-row state names are `samplingState.human_role` /
`model_role`; they lower to protocol `user_role` / `assistant_role` only in
`buildSamplingPayload`. Structural defaults are not sent as label overrides.
The old `speaking as` / `reply as` and segmented-seat description immediately
below documents the compatibility path, not the current composer UI.
