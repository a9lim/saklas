...
webui/src/
  ...
  panels/
    ...
    SteeringRack.svelte       # subspace group then manifold group of steer cards + "+ add steering"/"+ add manifold";
                              # empty state stacks both launchers below teaching copy
    ProbeRack.svelte          # subspace group then manifold group of probe cards + "+ add probe" / "+ add manifold probe" footer
  ...
  drawers/
    {SaveConversation,LoadConversation,Compare,SystemPrompt,
     Help,Export,Merge,Vectors,Extract,Manifold,ManifoldBuilder,
...
`ManifoldSteerCard` (every `manifoldRack` entry; accent + glyph follow the subspace/manifold family). A **flat** `%` manifold is **label-form only** — a snap-to-node `Select` plus an `along` slider, with no `XYPad`, because a flat `%` term raises on free coords at the engine. A **curved** manifold adds the `XYPad` (free coords; rendered `locked` while a label is bound, so the pad reads out but rejects input until `(free position)` unsnaps) plus an `onto` slider — the curved-only second coefficient that collapses the off-surface in-subspace residual onto the surface. Picking a node sets `ManifoldRackEntry.label` (label form, `<m>%<label>`); pulling the pad authors a free coord and clears it.
...
`VectorsDrawer` is the unified vector management surface — both rack `+ add` buttons (steering and probe) and `RecipeBuilderDrawer`'s "browse…" route here. It is re-sourced off `manifoldRack.catalog` filtered to the concept-axis manifolds (`node_count === 2 && fit_mode === "pca"`), split on `fitted_for_session` into "Extracted" vs "Statements only" and grouped by `categoryOf(m.tags)`:

- **Extracted** — concept axes with a baked tensor for the loaded model (`fitted_for_session`). Per row: `[ⓘ] [+steer] [+probe] [re-extract] [delete]`. The `[ⓘ]` toggle opens an inline metadata panel (description / source / recommended α / tags / tensor-models list) plus a `show layer norms →` deep-link to `LayerNormsDrawer` for the per-layer ||baked|| histogram + extraction-diagnostics stoplight. `[re-extract]` calls `/extract` with `force=true` — bypasses the tensor cache and re-runs the contrastive forward passes (parity with `ManifoldDrawer`'s `[re-fit]`). `[+probe]` calls `attachProbe("ns/name")`; `[delete]` calls `apiManifolds.delete`. The steer/probe buttons short-circuit when the concept is already racked.
- **Statements only** — concept axes with a node corpus on disk but no tensor for this model. Per row: `[extract] [delete]`. Extract reuses the cached corpus — one-click, no form, drives the same sticky-toast progress flow as ExtractDrawer. On success, refreshing flips `fitted_for_session` and reshuffles the row up into the "Extracted" section without remount.

The pack-backed `packsState` / `has_tensor` plumbing is gone — the drawer reads the live manifold catalog directly.
