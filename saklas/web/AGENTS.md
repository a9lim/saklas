...
- **GET `/sessions/{id}/vectors`** — registered steering vectors. Vector lifecycle now rides the session routes (`POST /extract`, `/vectors/merge`, `/vectors/clone`) + the manifold install/browse routes; there is **no `/saklas/v1/packs*` surface** (removed in the 4.0 collapse — see "Known server drift" below).
...
- **Pack routes are gone.** The server no longer serves `GET/POST /saklas/v1/packs`,
  ...
  moved to the manifold routes; vector extract/merge/clone stay under
  `/sessions/{id}/*`.
