...
On-disk format for manifold-steering artifacts — `~/.saklas/manifolds/<ns>/<name>/`, a peer of `vectors/` (a manifold is N labeled nodes on a domain, not a single bipolar concept, so not a `ConceptFolder`). `MANIFOLD_FORMAT_VERSION = 3` is decoupled from concept packs' `PACK_FORMAT_VERSION`; pre-v3 (1-D cyclic-spline) manifolds are rejected with a pointer to `scripts/upgrade_manifolds.py`. `ManifoldFolder.load` parses `manifold.json` (`name` / `domain` tagged union / `nodes` as `{label, coords}` objects / `files{sha256}`), validates the format version + `NAME_REGEX`, builds the domain via `domain_from_spec`, requires ≥ `min_nodes(n) = 2n+1` nodes, checks each node's `coords` arity against the domain's intrinsic dim, emits soft `UserWarning`s for poisedness deficiency and near-flat non-periodic axes, and verifies the `files` manifest when populated. `node_groups()` reads `nodes/NN_<label>.json` (one JSON statement list per node) in order; `nodes_sha256()` hashes the ordered corpus *plus* canonical-JSON of the domain and node coords (the per-tensor staleness key — a geometry edit invalidates it too). `write_metadata` re-hashes and rewrites `manifold.json` after a fit. The `files` manifest covers only fitted tensors + sidecars — the node corpus is user-editable (editing it is the re-fit trigger) so it is deliberately *not* integrity-hashed (`hash_manifold_files`). `ManifoldSidecar` is the lean per-tensor JSON (`method` / `domain` / `node_count` / `nodes_sha256` / optional SAE keys) — separate from `packs.Sidecar`, whose concept-extraction fields are meaningless here. Tensor save/load itself lives in `core/manifold.py`. `ManifoldFormatError` on any malformed folder.

`iter_manifold_folders(namespace=None)` is the shared discovery walk — yields `(namespace, ManifoldFolder)` for every installed manifold, skipping malformed folders; both the CLI (`_iter_manifold_folders` is a thin wrapper) and the server share it so neither re-implements the walk. `create_manifold_folder(namespace, name, description, domain_spec, nodes)` is the webui/HTTP authoring path: it validates name/namespace/labels against `NAME_REGEX`, builds the domain via `domain_from_spec`, enforces `min_nodes` and per-node coord arity + non-empty statement lists, writes `manifold.json` (with an empty `files` manifest) + the `nodes/` corpus, and returns `(folder, advisories)` — the captured `_warn_authoring_quality` warnings so the UI can flag a poisedness-deficient layout before a fit. `FileExistsError` on an existing folder, `ManifoldFormatError` on bad input. `update_manifold_folder(folder, *, description=None, nodes=None)` re-authors in place — a `nodes` argument fully replaces the node list and corpus; existing fitted tensors are kept (they go stale, the next fit overwrites).
