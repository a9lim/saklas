...
On-disk format for manifold-steering artifacts — `~/.saklas/manifolds/<ns>/<name>/`, a peer of `vectors/` (a manifold is N labeled nodes on a domain, not a single bipolar concept, so not a `ConceptFolder`). `MANIFOLD_FORMAT_VERSION = 5` is decoupled from concept packs' `PACK_FORMAT_VERSION`; pre-v3 (1-D cyclic-spline) manifolds are rejected with a pointer to `scripts/upgrade_manifolds.py`, and a *newer*-than-local `format_version` is rejected too (created-by-a-newer-saklas, "upgrade saklas") — the symmetric ceiling `PackMetadata.load` carries. v4 adds the optional per-layer `explained_variance_per_layer: {str(idx): float}` sidecar field used by additive-mode manifold steering's fit-quality normalization — bumping the version triggers `materialize_bundled_manifolds` to refresh the bundled manifest on next session start, and the EV field gets populated on the next fit; v3 sidecars load fine with empty EV, just without the quality-normalization boost. v5 adds the optional `origin_per_layer: {str(idx): [coord, ...]}` sidecar field (the per-layer authoring-coordinate foot of the neutral mean — each layer's cold-start foot seed — written by `core/manifold.save_manifold` only when set) — an absent field loads as an empty dict (the apply path seeds a zero-coord foot per layer), so the bump is back-compatible and just forces the bundled refit. The fitted-tensor sidecar also carries the optional `mahalanobis_share_per_layer: {str(idx): float}` (the whitened per-layer share, baked when the model whitener covers every fit layer) and a `share_metric: "mahalanobis"|"euclidean"` label — both fit-derived, written by `core/manifold.save_manifold`, absent → Euclidean-spread fallback; neither rides `ManifoldSidecar` (the cache-check dataclass) or bumps the format version. Two folder shapes share the format, discriminated by `manifold.json::fit_mode`:
...
`ManifoldFolder.load` validates format version (the manifold *name* against `NAME_REGEX`, each node *label* against the stricter `_LABEL_REGEX = ^[a-z][a-z0-9_-]{0,63}$` — labels drop `.` because the dot is reserved as the bipolar-pole separator and the steering-expr lexer addresses a label via `%label`, so a dotted label is neither typable nor resolvable; the error names it a "grammar-addressable identifier"), branches on `fit_mode`, enforces `min_nodes(n) = 2n+1` on authored folders (discover folders enforce it at fit time, once `k` is picked), verifies the `files` manifest when populated, and exposes the `is_discover` discriminator for downstream branching. The four label-validation sites (authored-load, discover-load, `_validate_authored_nodes`, `_validate_discover_corpora`) all share `_LABEL_REGEX`. `ManifoldFolder` carries a `source` field (`"local"` default / `"bundled"` / `"hf://<owner>/<name>[@rev]"`) mirroring `PackMetadata.source` — stamped by `pull_manifold`, preserved across re-fits by `write_metadata` (omitted from the manifest only when `"local"`, so non-HF folders stay byte-identical to the pre-source shape), and read by `refresh_manifold` to pick the re-pull path. `node_groups()` reads `nodes/NN_<label>.json` (one JSON statement list per node) in order; `nodes_sha256()` is the per-tensor staleness key — for authored folders it hashes `{corpus, domain, node_coords}`; for discover folders it hashes `{corpus, fit_mode, hyperparams}`. The field name is unchanged for back-compat with v3 sidecars (the spec called for a `fit_inputs_sha256` rename but the impl folds the new semantics into the existing key). `write_metadata` re-hashes and rewrites `manifold.json` after a fit. The `files` manifest covers only fitted tensors + sidecars — the node corpus is user-editable (editing it is the re-fit trigger) so it is deliberately *not* integrity-hashed (`hash_manifold_files`). Every fitted `.safetensors` must carry its `.json` sidecar — `ManifoldFolder.load` raises `ManifoldFormatError` on a sidecar-less tensor, the same invariant `ConceptFolder.load` enforces (fits always write one via `save_manifold`, so this only rejects a genuinely-corrupt folder). `ManifoldSidecar` is the lean per-tensor JSON (`method` / `domain` / `node_count` / `nodes_sha256` / optional SAE keys; plus `fit_mode` / `hyperparams` / `diagnostics` on discover fits) — separate from `packs.Sidecar`, whose concept-extraction fields are meaningless here. `ManifoldSidecar.method` round-trips `manifold_pca` / `manifold_sae` (authored) and `manifold_discover_pca` / `manifold_discover_spectral` / `manifold_discover_sae` (discover). Tensor save/load itself lives in `core/manifold.py` — discover fits additionally stash `node_coords` (the derived per-model layout) in the same safetensors. `ManifoldFormatError` on any malformed folder.

`iter_manifold_folders(namespace=None)` is the shared discovery walk — yields `(namespace, ManifoldFolder)` for every installed manifold, skipping malformed folders; both the CLI (`_iter_manifold_folders` is a thin wrapper) and the server share it so neither re-implements the walk. `create_manifold_folder(namespace, name, description, domain_spec, nodes)` is the authored webui/HTTP path: it validates name/namespace/labels against `NAME_REGEX`, builds the domain via `domain_from_spec`, enforces `min_nodes` and per-node coord arity + non-empty statement lists, writes `manifold.json` (with an empty `files` manifest) + the `nodes/` corpus, and returns `(folder, advisories)` — the captured `_warn_authoring_quality` warnings so the UI can flag a poisedness-deficient layout before a fit. `create_discover_manifold_folder(namespace, name, description, *, fit_mode, node_corpora, hyperparams=None, node_roles=None, scenarios=None)` is the discover-mode authoring path: nodes are `{label: [statements]}` with no coords, `fit_mode in {"pca","spectral"}` is the discriminator, and `_sanitize_hyperparams(fit_mode, hyperparams)` drops cross-method keys (e.g. `k_nn` on a PCA folder) at the IO boundary so a stray hyperparam never lands in the manifest and crashes the dispatcher at fit time. `scenarios` (optional) persists the generation-provenance list to `scenarios.json` (`{"scenarios": [...]}`, the same schema vector extraction writes) so a later re-fit can regenerate against the same domains. A **streaming** companion writes one node at a time for big-roster generation, where holding every corpus in memory at once is wasteful and a crash should keep the nodes already finished: `init_discover_manifold_folder(namespace, name, description, *, fit_mode, labels, hyperparams=None, node_roles=None)` writes the label-only skeleton + empty `nodes/`, then `append_discover_manifold_node(folder, index, label, statements)` drops each `nodes/NN_<label>.json` as it completes (atomic per-file, no whole-corpus staging swap — that's the point), and `write_manifold_scenarios(folder, scenarios)` lands `scenarios.json`. The regen scripts wire these to `generate_statements`'s `on_corpus` / `on_scenarios` sinks; the SIGKILL-survives-with-finished-nodes property comes from the streaming write + a stable staging path (the user-facing surfaces write straight to `manifold_dir(ns, name)`; the bundled regen writes straight to the package data tree). `plan_discover_generation(folder, name, description, *, fit_mode, labels, hyperparams=None, node_roles=None)` is the shared **resume / add-nodes** planner every generate surface (`vector manifold generate`, the HTTP generate route, the regen scripts) calls: it creates the skeleton when absent, **extends** `manifold.json` with any labels new to an existing folder (add-nodes — the existing fit goes stale on the next read since the corpus hash changes), and returns `DiscoverGenerationPlan(folder, index_of, pending, scenarios, added, resumed)` where `pending` is the declared labels whose `nodes/NN_<label>.json` is absent and `scenarios` is the locked list from `scenarios.json` (so a resumed run regenerates the missing nodes against the original domains). The read is deliberately lenient — it does **not** route through `ManifoldFolder.load`, which rejects a partial by the missing-node-corpus guard (lines that check every declared label has a file) — so it can inspect the very partial it is resuming. `read_manifold_scenarios(folder)` is the `{"scenarios": [...]}` reader behind it. The generate surfaces compute `pending` minus their anchor, generate only those (named-by-default — a single pending node still gets concept naming), and write the anchor from the bundled neutrals without the model. `update_manifold_folder(folder, *, description=None, nodes=None)` re-authors authored folders in place — a `nodes` argument fully replaces the node list and corpus; existing fitted tensors are kept (they go stale, the next fit overwrites). `FileExistsError` on an existing folder, `ManifoldFormatError` on bad input.
...
`transfer_manifold(folder, *, from_model, to_model, alignment, transfer_quality_estimate=None, whitener=None, layer_means=None, force=False)` is the manifold analogue of `vector transfer` — pure-io: the caller supplies the per-layer Procrustes `alignment` map (`{layer: M_L}`, the shape `alignment.fit_alignment` produces; building it needs both models loaded, a session/CLI concern), and this function only *applies* it. It loads the source fit at `<folder>/<safe_from>.safetensors`, maps each covered layer's affine subspace (`mean → M_L @ mean`, `basis → basis @ M_L^T` — both live in model space) into target space, leaves the RBF interpolant + `node_coords` untouched (they're subspace/authoring-coordinate-space, invariant under the model-space map), drops uncovered layers (mirrors `transfer_profile`), and `save_manifold`s to the `_from-<safe_src>` filename variant (`paths.tensor_filename(to_model, transferred_from=from_model)`). **Target-metric re-bake:** when a **target** `whitener` (+ `layer_means`) covering every transferred layer is supplied, the per-layer Mahalanobis share + steering lever are recomputed in target space (share via `subspace_gram(transferred_basis)`, lever via `layer_lever` over the target neutrals) and `share_metric: "mahalanobis"` is stamped; without it both are cleared (Euclidean spread + `N=1` at apply, `share_metric: "euclidean"`) — Σ and the neutrals are per-model so the source values are invalid in target space. `origin` is always cleared to an empty dict (it's the per-layer foot of the *source* neutral mean, invalid in target space — same reason; the apply path falls back to a zero-coord seed per layer). `subspace_metric` carries the source value (the basis was selected on the source and only rotated). The CLI `vector manifold transfer` runner builds the target whitener from the target neutral cache (centered by its own mean) and threads both in, mirroring `_run_transfer`. The sidecar is patched post-save with `source_model_id` + `transfer_quality_estimate` (`save_manifold`'s fixed key allow-list doesn't carry them), method tagged `manifold_procrustes_transfer`, and the folder `files` manifest re-hashed. `FileNotFoundError` (no source fit), `ManifoldFormatError` (empty alignment / no covered layer), `FileExistsError` (transferred tensor exists, no `force`).
...
prefixes, and a trailing `:variant` (`raw` | `pca` | `sae[-<release>]` |
`role[-<id>]` | `from[-<src>]`, via `_VARIANT_REGEX`). `resolve(selector)` walks `vectors_dir()` into
...
install snippet is `saklas vector manifold install <coord>`, not `pack install` —
a manifold repo has no `pack.json`); the
...
transfer_quality_estimate=None, whitener=None)` applies `M_L @ v_src` per layer
(uncovered layers dropped), tagging the result `method="procrustes_transfer"`.
With a **target** `whitener` covering every transferred layer (all-or-nothing),
each layer's magnitude is re-scaled to its *target* Mahalanobis norm
(`v' = v_tgt · ‖v_tgt‖_M(target) / ‖v_tgt‖₂`, direction preserved) so the
cross-layer share the angular hook reads back is in the target metric instead of
the inherited source one; `bake` is stamped `"mahalanobis"` (else `"euclidean"`).
The CLI `_run_transfer` builds the target whitener from the target neutral cache
(centered by its own mean — the probe baseline — so no `layer_means` cache is
needed) and threads it in.
