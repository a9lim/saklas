...
- `GET /saklas/v1/manifolds` — every installed manifold with domain spec, intrinsic dim, node labels/coords, `min_nodes`, `fit_mode` discriminator, `hyperparams` (discover-mode), `fitted_for_session` (a tensor for the loaded model exists), and `stale` (the fitted tensor's `nodes_sha256` no longer matches the corpus). Discover folders report `domain == {}` and `node_coords == []` at the list level — the derived per-model layout lives on the detail route.
