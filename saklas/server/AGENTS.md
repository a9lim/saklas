...
- `POST /saklas/v1/manifolds/generate` body `{namespace, name, description, concepts, n_scenarios?, statements_per_concept?, fit_mode?, hyperparams?, force?, role_per_node?}` — LLM-authors a discover folder by wrapping `session.generate_concept_statements` under the session lock; SSE progress when `Accept: text/event-stream`, JSON otherwise. `role_per_node=true` doubles each concept slug as that node's assistant-role substitution at fit time, producing a persona manifold. Writes a fresh discover folder + `scenarios.json` provenance; pair with `POST .../fit` to derive coords + fit.
