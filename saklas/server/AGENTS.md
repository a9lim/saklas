...
- `POST /saklas/v1/manifolds` body `{namespace, name, description, domain, nodes:[{label,coords,statements}]}` — authors an *authored* artifact via `io.manifolds.create_manifold_folder`; `domain` is box or sphere only. Returns the detail shape plus `advisories` (soft poisedness warnings). 409 on an existing folder, 400 on malformed input.
- `POST /saklas/v1/manifolds/discover` body `{namespace, name, description, fit_mode, nodes:[{label,statements}], hyperparams?}` — authors a *discover* artifact via `create_discover_manifold_folder`; nodes carry no coords (derived per-model at fit time). `_sanitize_hyperparams` drops cross-method keys at the IO boundary.
- `POST /saklas/v1/manifolds/generate` body `{namespace, name, description, concepts, n_scenarios?, statements_per_concept?, fit_mode?, hyperparams?, force?}` — LLM-authors a discover folder by wrapping `session.generate_concept_statements` under the session lock; SSE progress when `Accept: text/event-stream`, JSON otherwise. Writes a fresh discover folder + `scenarios.json` provenance; pair with `POST .../fit` to derive coords + fit.
