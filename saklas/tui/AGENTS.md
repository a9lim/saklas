...
- Probes: `/probe <concept|pos . neg|ns/>`, `/unprobe <name|ns/>`, `/extract <concept|pos . neg> [--role <slug>]` (cache-warm only — and the only path that creates a new bipolar concept from `<pos> <neg>`).
- Custom extraction: `/pairs <name> [--role <slug>]` opens `CustomPairsModal` (`pairs_modal.py`) — a multi-line `TextArea`, one `positive | negative` pair per line, `Ctrl+S` (or `Ctrl+Enter`) extract / `Esc` cancel. Submitted pairs go straight to `session.extract` as a `DataSource`, skipping scenario/pair generation. The modal can't be a `PendingItem`, so `/pairs` is refused mid-generation rather than queued.
- Role-augmented extraction: `--role <slug>` is peeled off `/extract` and `/pairs` args by `SaklasApp._peel_role_flag` before the bipolar parser runs (so multi-word poles don't collide with the trailing flag), and forwarded to `session.extract(role=...)`. A `:role-<slug>` variant typed directly on a `/steer` term (`/steer 0.5 angry:role-pirate`) also triggers a fresh role-augmented extraction when the tensor isn't cached — same shape as `:sae-<release>`. The variant suffix wins over an explicit `--role` flag because it rides through `resolve_pole`'s `explicit_variant` arm. Slug validation lives in `core.role_templates`; family unsupported (Mistral-3 / talkie) raises `RoleSubstitutionUnsupportedError` at extract time.
- Manifold: `/manifold fit <folder>` fits an authored manifold pack via `session.extract_manifold` (gated like `/extract` — refused during A/B shadow, deferred behind an in-flight gen as a `manifold_fit` pending item). Manifold authoring stays out of the TUI. A `%` manifold term in `/steer` (`<coeff> <manifold>%<position>` — coord form `0.3,0.8` or label form `pirate`) routes through `_dispatch_manifold_term`, which eager-loads + validates the artifact (`session._ensure_manifold_loaded`, then `Manifold.resolve_position` to handle both forms + arity vs the domain's `intrinsic_dim`) and stashes the `ManifoldTerm` on `app._manifold_terms` — a parallel dict to `_alphas` because a `ManifoldTerm` is not a `Profile`. Both dicts share `_enabled` and are merged in `_active_alphas` into `Steering.alphas` (`AlphaEntry` already admits `ManifoldTerm`). A mixed expression's vector siblings still apply (`continue`, not `return`, on a manifold term). The left-panel manifold row renders in `ansi_magenta` to differentiate it from vector rows.

A bare slug typed into `/steer` (`/steer 0.7 pirate`) resolves through `io.selectors.resolve_bare_name`: first as an installed bipolar pole, then — if that misses — as a manifold node label. A unique manifold-label match synthesizes a label-form `ManifoldTerm` (`/steer 0.7 pirate` → `local/persona%pirate`). Cross-tier ambiguity raises `AmbiguousSelectorError`. Persona manifolds (per-node `role` set) also drive role-paired steering: at scope entry `session.steering()` aggregates each manifold term's `manifold.nearest_node_role(position)` and pipes the winning role through `session._active_role` so the prefill applies the chat-template substitution. Disagreement (multiple terms imply different roles) is soft-warn + highest-`|coeff|`-wins (`RoleBaselineMismatchWarning`); explicit `:role-<X>` on a vector term overrides any manifold-implied role with the same warning.
