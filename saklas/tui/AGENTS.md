...
Raw mode is `chat_panel.RawBuffer` — the TUI mirror of `webui`'s `RawBuffer.svelte`. The loom active path is flattened (`tree.flat_text()`) into one editable `_RawTextArea`; `Enter` posts `RawBuffer.Continue`, `Shift+Enter`/`Ctrl+J` insert a newline. `RawBuffer` implements the `_AssistantMessage` *streaming protocol* (`append_token`, `append_token_score`, `set_token_data`, `apply_highlight`, …) so `_poll_generation` routes a generation into it as the stream `widget` unchanged. `_start_generation(..., raw_continuation=True)` is the raw path: the divergence between the edited draft and the committed tree is resolved by `_resolve_raw_divergence` (port of the webui's `resolveDivergence`) into a `(tail, parent_node_id)` pair fed to `generate_stream(input=tail, raw=True, parent_node_id=parent)`. Per-token highlight survives via a tinted `Static` mirror layered (CSS `layers: base over`) over the textarea, shown only while the buffer is clean — exactly the webui `showTint` rule. The chat input stays visible in raw mode as the command line: a `/`-prefixed line runs a slash command; a plain line appends + continues. `raw=self._raw_mode` rides on every `generate_stream` call, so the engine feeds the model `tree.flat_text()` verbatim rather than the `build_chat_input` `User:/Assistant:` marker fallback. Known v1 rough edges: A/B, auto-regen, regen, and `Ctrl+Enter` commit are chat-mode only (gated off in raw); a dirty mid-buffer edit is discarded on navigation / mode-switch unless `Enter`'d; tree mutations outside the `_repaint`/`clear`/`rewind` funnels (`/edit`, `/branch`, …) don't re-sync the raw buffer until the next navigation. The loom screen still shows engine role tags — out of scope.
