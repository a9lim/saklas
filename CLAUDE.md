...
- `vectors.py` — Two extraction methods: `extract_actadd` (concept and baseline run as **separate forward passes** to avoid batch contamination from fully-masked padding — critical when the baseline tokenizes to zero valid tokens) and `extract_caa` (contrastive pairs batched into a single forward pass). Batched variant `extract_actadd_batched` for probe bootstrap (ensures baseline has ≥1 real token via BOS fallback). All extraction uses `torch.inference_mode()` with `use_cache=False`. When `layers` is passed, extraction captures hidden states via a forward hook on the target layer with `torch.mps.synchronize()` before clone (robust for multimodal wrappers where `output_hidden_states` indexing differs from language model layer indices, and for MPS async). Vectors saved as `.safetensors` + `.json` metadata sidecar.
...
- **Vector extraction math**: Difference vectors are computed as `pos - neg` directly. Do not re-introduce mean-centering — it's algebraically redundant (`(pos - center) - (neg - center) == pos - neg`). CAA uses a single batched forward pass for all positive+negative pairs. Vectors are scaled to 10% of the mean hidden-state norm at the extraction layer, so alpha=1.0 gives a ~10% perturbation. This works uniformly across architectures: standard models (hidden norms ~50-200) and Gemma 4 (hidden norms ~1e-5 due to cumulative `layer_scalar`). **All norm computations in extraction use `.float()` to avoid fp16 overflow** — for hidden_dim=2048 with element magnitudes ~6, the sum-of-squares (73728) exceeds fp16 max (65504), producing Inf and zeroing the vector. This is a hard requirement, not an optimization.
