...
- `vectors.py` — Separate forward passes per prompt (no batching — multimodal models produce corrupted hidden states with padded batches). `_capture_all_hidden_states` hooks every layer in a single forward pass, returning `{layer_idx: hidden_state}`. `_encode_and_capture_all` handles tokenization, chat-template wrapping (instruct models get assistant-role framing; base models get raw strings), BOS fallback, and attention-weighted pooling (last token's self-attention from the last layer, averaged across heads; falls back to last-token pooling). `extract_contrastive`: 2N passes for N contrastive pairs, captures all layers per pass. Per-layer SVD extracts the first principal component; each layer is scored by explained variance ratio (σ₁/Σσᵢ). Returns a **profile**: `dict[int, (Tensor, score)]` mapping every layer to its direction vector and signal strength (no layers dropped). Single-pair case uses raw diff norm as score proxy. Profiles saved as multi-tensor `.safetensors` (keys `layer_{i}`) + `.json` sidecar with scores. Cache filenames are sanitized (`re.sub` on unsafe chars), format: `{concept}.safetensors` (no layer suffix).
...
- `monitor.py` — `TraitMonitor` accepts probe profiles. Monitors **all layers** in each probe's profile, weighted by score. Hooks are grouped by layer — one hook per distinct layer across all probe profiles. All hooks write into a shared GPU accumulator `(max_tokens, num_probes)` via score-weighted cosine similarities; the highest-layer hook advances `buf_idx`. `flush_to_cpu()` divides by precomputed total weights to produce a weighted average, then transfers to CPU history and running stats. `has_pending_data()` gates TUI polling. `add_probe`/`remove_probe` accept profiles and trigger full hook rebuild.
...
- **Monitor hooks**: One hook per distinct layer across all probe profiles, writing score-weighted cosine similarities into a shared GPU accumulator `(max_tokens, num_probes)`. Per-hook: branchless `last_state / norm.clamp(min=1e-8)` — no CPU sync. Highest-layer hook advances shared `buf_idx`. `flush_to_cpu()` divides by total weights, vectorizes stats across probe dimension. TUI polls gated on `has_pending_data()`. History uses `deque(maxlen=8)` to cap memory (only sparkline and current/previous consume it).
