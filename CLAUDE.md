...
- **Monitor uses capture hooks**: lightweight single-slot-overwrite hooks run during generation to capture last-token hidden states per layer. Probe scoring uses these directly (`measure_from_hidden`) — no separate post-generation forward pass. Mean-centered cosine similarities (both norms) remove baseline bias, bounded to [-1, 1]. Capture hooks are registered after steering hooks, so they see steered activations.
